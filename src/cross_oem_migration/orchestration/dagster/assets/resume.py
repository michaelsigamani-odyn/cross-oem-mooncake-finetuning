...
def resumed_checkpoint(context: AssetExecutionContext, settings: SettingsResource, executor: ExecutorResource, data: DataResource) -> Dict[str, Any]:
    ...
    run_root = f"{cfg.remote_root}/nvidia_run_{cfg.run_id}"
    ...
    spec = FineTuningJobSpec(
        ...
        use_gpu=cfg.target_use_gpu, resume_from_checkpoint=f"{cfg.remote_root}/checkpoint-{cfg.checkpoint_step}",
    )
    python_cmd = cfg.host_python_cmd_overrides.get(cfg.target_host, cfg.target_python_cmd)
    command = workload.build_command(python_cmd=python_cmd, remote_root=cfg.remote_root, spec=spec)
...
def resume_validation(context: AssetExecutionContext, settings: SettingsResource, executor: ExecutorResource) -> Dict[str, Any]:
    ...
    python_cmd = cfg.host_python_cmd_overrides.get(cfg.target_host, cfg.target_python_cmd)
    command = (
        f"{python_cmd} {cfg.remote_root}/scripts/validation/validate_resume.py "
        f"--source-checkpoint {cfg.remote_root}/checkpoint-{cfg.checkpoint_step} "

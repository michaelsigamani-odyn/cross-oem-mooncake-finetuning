...
from .preflight import source_gpu_preflight


@asset(group_name="training", deps=[source_gpu_preflight], description="Fine-tunes the base model on the source host up to `checkpoint_step`.")
def source_checkpoint(context: AssetExecutionContext, settings: SettingsResource, executor: ExecutorResource, data: DataResource) -> Dict[str, Any]:
    ...
    run_root = f"{cfg.remote_root}/amd_run_{cfg.run_id}"

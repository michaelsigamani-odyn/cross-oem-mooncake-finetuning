"""The initial preflight probe necessarily self-detects vendor (hip vs.
cuda) because we don't know what's on a host until we ask it. This is a
direct, unmodified port of `kernel_preflight_script` from the original
file (lines 2030-2142) -- it is NOT duplicated per-vendor, it's shared
infrastructure that both NvidiaAdapter and AmdAdapter delegate to for the
*first* probe. Everything downstream of that first probe (telemetry
sampling, torch install command) IS vendor-specific and lives in
nvidia.py / amd.py.
"""

SHARED_PREFLIGHT_SCRIPT = r"""
